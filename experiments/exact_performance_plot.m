...
N = 15000;
Nts = 10;
mu = 1e-10 * N; % Change to generate different plots (1e-10 -- 1e-7)
bandwidth = 3;
num_iter = 250;
kernel = "gaussian";
...
for k = 1:numel(names)
    ...
    semilogy(results.(names{k}).rls(:,2), 'Color', color6,'LineStyle', '-', 'Marker','+')
    ...
    semilogy(results.(names{k}).rls(:,1), 'Color', color6,'LineStyle', '-')
...
plot(cumulative(:, 2)/numberproblems, 'Color', color6, 'LineStyle', '-') % RLS
...
%le = legend({'Greedy', 'Uniform','RLS','RFF','No Preconditioner', 'RPCholesky (Ours)'}, 'Location', 'northwest');
...
accuracy = 1e-3
difficulty = ones(numel(names), 5) * 250;
for k = 1:numel(names)
    add = (results.(names{k}).rpc(:,1) <= accuracy);
    if max(add) == 1
        difficulty(k, 1) = min(find(add));
    end
    add = results.(names{k}).rls(:,1) <= accuracy;
    if max(add) == 1
        difficulty(k, 2) = min(find(add));
    end
    add = results.(names{k}).greedy(:,1) <= accuracy;
    if max(add) == 1
        difficulty(k, 3) = min(find(add));
    end
    add = results.(names{k}).uniform(:,1) <= accuracy;
    if max(add) == 1
        difficulty(k, 4) = min(find(add));
    end
    add = results.(names{k}).rff(:,1) <= accuracy;
    if max(add) == 1
        difficulty(k, 5) = min(find(add));
    end
end
mean(difficulty,2)
(difficulty(:,2)-difficulty(:,1))./difficulty(: